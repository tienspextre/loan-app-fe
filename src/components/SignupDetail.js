...
export default function SignupForm() {
  ...
  useEffect(() => {
    ...
    const fetchProvinces = () => {
      fetch('http://localhost:8080/api/provinces', { headers })
        .then(response => {
          if (!response.ok) {
            throw new Error('Failed to fetch data');
          }
              ...
              <form onSubmit={handleSignup}>
                ...
                <div className="mb-3">
                  ...
                  <select
                    id="inputProvince"
                    className="form-control"
                    value={province} // Set the value of the select to the selectedBranch state
                    onChange={handleProvinceChange}
                    required // Call handleBranchChange when the value changes
                  >
                    <option value="-1">Chọn Tỉnh</option>
                    {province.map(option => (
                      <value key={option.id} value={option.id}>
                        {option.name}
                      </value>
                    ))}
                  </select>
